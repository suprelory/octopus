...
func TestDBExportThenImportRoundtrip(t *testing.T) {
	...
	// Create test data
	proxy := &model.ProxyConfiguration{Name: "roundtrip-proxy", URL: "http://127.0.0.1:8080", Enabled: true}
	if err := ProxyConfigurationCreate(proxy, ctx); err != nil {
		t.Fatalf("ProxyConfigurationCreate failed: %v", err)
	}
	channel := &model.Channel{
		Name: "roundtrip-channel", Type: outbound.OutboundTypeOpenAIEmbedding,
		ProxyMode: model.ProxyUsageModePool, ProxyConfigID: &proxy.ID,
	}
	...
	// Occupy the source ID so every imported reference must be remapped.
	existingProxy := &model.ProxyConfiguration{ID: proxy.ID, Name: "existing-proxy", URL: "http://127.0.0.1:9090", Enabled: true}
	if err := ProxyConfigurationCreate(existingProxy, ctx); err != nil {
	...
	var freshProxy model.ProxyConfiguration
	if err := dbpkg.GetDB().Where("url = ?", proxy.URL).First(&freshProxy).Error; err != nil {
		t.Fatalf("query imported proxy: %v", err)
	}
	if freshProxy.ID == proxy.ID {
		t.Fatal("import reused an ID belonging to a different proxy")
	}
	var freshChannel model.Channel
	if err := dbpkg.GetDB().Where("name = ?", channel.Name).First(&freshChannel).Error; err != nil {
		t.Fatalf("query imported channel: %v", err)
	}
	if freshChannel.Type != outbound.OutboundTypeOpenAIEmbedding {
		t.Fatalf("channel type changed: %d", freshChannel.Type)
	}
	if freshChannel.ProxyMode != model.ProxyUsageModePool || freshChannel.ProxyConfigID == nil || *freshChannel.ProxyConfigID != freshProxy.ID {
		t.Fatalf("channel lost its proxy reference: %+v", freshChannel)
	}
	if freshSite.ProxyMode != model.ProxyUsageModePool || freshSite.ProxyConfigID == nil || *freshSite.ProxyConfigID != freshProxy.ID {
		t.Fatalf("site lost its proxy reference: %+v", freshSite)
	}
	var proxyAccountCount int64
	if err := dbpkg.GetDB().Model(&model.SiteAccount{}).
		Where("site_id = ? AND proxy_mode = ? AND proxy_config_id = ?", freshSite.ID, model.ProxyUsageModePool, freshProxy.ID).
		Count(&proxyAccountCount).Error; err != nil {
		t.Fatalf("count account proxy references: %v", err)
	}
	if proxyAccountCount != 5 {
		t.Fatalf("expected 5 remapped account proxy references, got %d", proxyAccountCount)
	}
}
...
func buildTestDump() *model.DBDump {
	return &model.DBDump{
		...
		Groups: []model.Group{
			{ID: 1, Name: "test-group", Mode: model.GroupModeRoundRobin},
		},
...
func TestDBImportSkipsReferencesToExistingUnsupportedChannel(t *testing.T) {
	ctx := setupBackupTestDB(t)
	legacy := model.Channel{Name: "existing-legacy-volcengine", Type: outbound.OutboundType(-1), Enabled: false}
