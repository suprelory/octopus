...
import { type SiteChannelTableSort, type SiteChannelTableSortField } from './ui-store';
import { type SiteModelView, formatHistoryTime, routeSourceLabel, routeTypeLabel } from './utils';
...
>(function SiteChannelTableView(
    ...
    useImperativeHandle(
        ...
        () => ({
            scrollToModelKey: (key: string) => {
                const index = models.findIndex((model) => makeModelKey(model.group_key, model.model_name) === key);
                if (index >= 0) {
    ...
    return (
        <div ref={scrollRef} role="table" className="h-full w-full overflow-auto overscroll-contain">
            <div className="min-w-[74rem]">
                ...
                >
                    ...
                    <div role="columnheader" className="text-right text-xs font-medium text-muted-foreground">
                        操作
                    </div>
                </div>
                            ...
                            >
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <div className="flex min-w-0 items-center gap-1.5">
                                                ...
                                                {model.source === 'manual' ? (
                                                    <Badge
                                                        variant="outline"
                                                        className="h-5 shrink-0 px-1.5 text-[10px] border-primary/30 bg-primary/10 text-primary"
                                                    >
                                                        自定义
                                                    </Badge>
                                                ) : null}
                                ...
                                <div role="cell" className="min-w-0">
                                    <div className="flex flex-wrap gap-1.5">
                                        <Badge
                                            variant="outline"
                                            className={cn('h-6 px-2 text-[11px]', getRouteTypeTone(model.route_type))}
                                        >
                                        ...
                                        {isSupportedRouteType(model.route_type) &&
                                        model.route_metadata?.route_guessed ? (
                                            <Badge
                                                variant="outline"
                                ...
                                <div role="cell" className="min-w-0">
                                    <Badge
                                        variant="outline"
                                        className={cn('h-6 px-2 text-[11px]', getRouteSourceTone(model.route_source))}
                                    >
                                ...
                                <div role="cell" className="min-w-0">
                                    <div className="flex flex-wrap gap-1.5">
                                        {model.disabled ? (
                                            <Badge
                                                variant="outline"
                                                className="h-6 px-2 text-[11px] border-destructive/30 bg-destructive/10 text-destructive"
                                            >
                                                已禁用
                                            </Badge>
                                        ) : (
                                            <Badge
                                                variant="outline"
                                                className="h-6 px-2 text-[11px] border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300"
                                            >
                                                已启用
                                            </Badge>
                                        )}
