import { type SiteChannelGroup, type SiteModelRouteType } from '@/api/endpoints/site-channel';
import { isSupportedRouteType } from './constants';
import { type SiteChannelQuickFilter, type SiteChannelTableSort } from './ui-store';
import { type SiteModelView, routeTypeLabel } from './utils';
...
export function sortModels(models: SiteModelView[], tableSort: SiteChannelTableSort) {
    return [...models].sort((left, right) => {
        switch (tableSort.field) {
            case 'group_name':
                return compareText(
                    left.group_name || left.group_key,
                    right.group_name || right.group_key,
                    tableSort.order,
                );
            case 'route_type':
            ...
            case 'last_request_at':
                return compareNullableNumber(
                    getModelLastRequestAt(left),
                    getModelLastRequestAt(right),
                    tableSort.order,
                );
            case 'model_name':
            default:
...
export function getGroupStatusBadge(group: SiteChannelGroup): { label: string; className: string } | null {
    ...
    if (!group.has_keys) {
        return {
            label: '待建',
            className: 'rounded-full bg-amber-500/15 px-1.5 py-0.5 text-[10px] text-amber-700 dark:text-amber-300',
        };
    }
